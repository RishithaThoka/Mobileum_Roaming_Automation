...
router.get('/sla', async (req, res) => {
  try {
    ...
    const workflows = await db('document_workflow_state as ws')
      .select([
        'ws.id', 'ws.document_id', 'ws.stage_status', 'ws.updated_at',
        'd.operator_id', 'd.created_at as ingested_at',
      ])
      .join('documents as d', 'ws.document_id', 'd.id');
    ...
    let totalRollbacks = 0, totalCompleted = 0, totalOnTime = 0;
    const operatorsInBreach = new Set();

    const opStats = {};
    operators.forEach(op => {
      opStats[op.id] = {
        operator: op, total_processed: 0, audited: 0, rejected: 0,
        rolled_back: 0, cycle_time_sum_ms: 0, cycle_time_count: 0,
        recent_outcomes: [], breaching: false,
      };
    });
    ...
    workflows.forEach(wf => {
      ...
      if (isTerminal) {
        if (wf.stage_status === 'deployed') {
          stats.audited++; totalCompleted++;
          if (durationMs <= slaThresholdMs) totalOnTime++;
        } else if (wf.stage_status === 'rejected') {
          stats.rejected++;
        } else if (wf.stage_status === 'rolled_back' || wf.stage_status === 'failed') {
          stats.rolled_back++; totalRollbacks++;
        }
        stats.cycle_time_sum_ms += durationMs;
        stats.cycle_time_count++;
        stats.recent_outcomes.push({ status: wf.stage_status, date: wf.updated_at, ms: updatedMs });
      } else {
        if (now - ingestedMs > slaThresholdMs) {
    ...
    const opList = Object.values(opStats).map(stats => {
      ...
      return {
        id: stats.operator.id, name: stats.operator.name,
        country: stats.operator.country, region: stats.operator.region,
        total_processed: stats.total_processed, audited: stats.audited,
        rejected: stats.rejected, rolled_back: stats.rolled_back,
    ...
    res.json({ platform, operators: opList, cross_partner });
  } catch (error) {
